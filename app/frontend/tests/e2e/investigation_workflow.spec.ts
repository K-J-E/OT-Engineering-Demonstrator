...
test('v1.0 consequence is investigated before same-build v1.1 repeat and regression', async ({ page }) => {
  ...
  await expect(page.getByTestId('same-build-proof')).toHaveText('Same backend-controlled application build proven across v1.0 failure and v1.1 direct repeat.')
  await expect(page.getByTestId('same-build-proof')).not.toContainText('corrected regression')
  ...
  await expect(page.getByTestId('same-build-proof')).toHaveText('Same backend-controlled application build proven across v1.0 failure, v1.1 direct repeat and corrected regression.')
