...
export function SafetyValidationResult({ projection, onReturn, onExit }: { projection: WorkspaceProjection; onReturn: () => void; onExit: () => void }) {
  ...
  return <div className="view-stack">
    <section className="panel" aria-labelledby="safety-result-title">
      ...
      <div className="guided-continuation"><div><strong>No defect investigation is required</strong><p>The withheld action is the accepted safe result, not an unexplained failure. The telemetry page remains available if you want to review the exact stale timestamps again.</p></div><div className="safety-result-actions"><button type="button" className="secondary-action" onClick={onReturn}>Return to stale telemetry evidence</button><button type="button" className="primary-action" onClick={onExit}>Return to walkthrough selection</button></div></div>
    </section>
  </div>
}
