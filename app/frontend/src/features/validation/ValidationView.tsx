...
function ExplorationValidationProcedure({ projection, definitions, summary, actions, busy, onAction }: {
  ...
  return <section className="panel validation-procedure" aria-labelledby="exploration-procedure-title">
    <div className="panel-heading"><div><span className="eyebrow">Validation procedure</span><h2 id="exploration-procedure-title">How the exploration evidence is controlled and extended</h2></div><span className={`status-badge ${summary?.execution.status === 'FINALISED' ? 'success' : 'neutral'}`}>{summary?.execution.status === 'FINALISED' ? 'Current result complete' : 'Current result in progress'}</span></div>
    ...
    <div className="procedure-grid" aria-label="Exploration validation procedure">
    ...
    <div className="current-validation-contribution">
      <div><span className="eyebrow">Current run contribution</span><h3>{currentCase?.caseDefinition.case_title ?? `${projection.run.fault_section_id} controlled exploration result`}</h3></div>
      ...
      <p>{currentCase === undefined ? 'This run remains a separately identifiable exploration record.' : `This run contributes the ${currentCase.caseDefinition.case_id} constituent case to “${campaignTitle(currentCase.definition)}”. It does not imply that the other required scenarios have been executed.`}</p>
    </div>

    <section className="campaign-section" aria-labelledby="multi-scenario-campaigns-title">
      <div className="section-heading"><div><span className="eyebrow">Cross-scenario assurance</span><h3 id="multi-scenario-campaigns-title">Multi-scenario validation campaigns</h3></div><p>Coverage is calculated from finalised, separately preserved exploration executions in this evidence history.</p></div>
...
function ExplorationValidation({ projection, busy, onAction, onContinue }: {
  ...
  return <div className="view-stack">
    ...
    <section className="panel" aria-labelledby="exploration-stage-validation-title">
      ...
      <p>These are the live network, telemetry and safety checks used by the operating logic while this exploration is running. They determine which actions can proceed; they are not the secondary validation verdict.</p>
    ...
    <section className="panel validation-progress" aria-labelledby="exploration-evidence-title">
      <div className="panel-heading"><div><span className="eyebrow">Exploration system-validation report</span><h2 id="exploration-evidence-title">Validation of the operating logic for this case</h2></div><span className="status-badge exploratory">Separate exploration record</span></div>
      <p>This secondary layer checks whether the operating mechanism produced the accepted case-specific result from the selected fault, configuration and telemetry. It is designed to reveal incorrect source relationships, isolation boundaries, customer calculations or restoration decisions. The result remains separate and is never counted as formal evidence.</p>
...
export function ValidationView({ projection, busy, onAction, onContinue }: { projection: WorkspaceProjection; busy: boolean; onAction: (action: ValidationWorkspaceAction) => void; onContinue: (view: 'operational' | 'restoration') => void }) {
  ...
  return <div className="view-stack">
    ...
    <section className="panel" aria-labelledby="stage-validation-title">
      ...
      <p className="traceability-pointer">The saved operating states become input to the separate <strong>system validation</strong> report below, which tests whether this operating mechanism produced the accepted formal result.</p>
    </section>

    <section className="panel validation-progress" aria-labelledby="validation-progress-title">
      <div className="panel-heading"><div><span className="eyebrow">Formal system-validation report</span><h2 id="validation-progress-title">Validation of the operating logic</h2></div><span className="status-badge formal">{savedCheckpoints.size} of 6 states saved</span></div>
    ...
    <section className="panel validation-procedure" aria-labelledby="formal-procedure-title">
      <div className="panel-heading"><div><span className="eyebrow">Validation procedure</span><h2 id="formal-procedure-title">How the formal result is produced</h2></div><span className={`status-badge ${summary?.execution.status === 'FINALISED' ? 'success' : 'neutral'}`}>{summary?.execution.status === 'FINALISED' ? 'Procedure complete' : 'Procedure in progress'}</span></div>
      <p>The formal verdict is not assigned from the final screen alone. It is produced from controlled entry conditions, six separately preserved network states and the accepted comparison method.</p>
      <div className="procedure-grid" aria-label="Formal validation procedure">
        <article><span className="procedure-number">1</span><div><h3>Lock the validation basis</h3><p>The application version, network configuration, accepted test definition, catalogue and formal scenario identity are bound before evidence is accepted.</p></div></article>
    ...
    <section className="panel" aria-labelledby="execution-title">
      ...
      {summary === undefined ? <><p className="empty-state">The evidence record has not been created yet.</p>{startAction !== undefined && <button type="button" className="primary-action" disabled={busy} onClick={() => onAction(startAction)}>Create evidence record</button>}</> : <>
        <p>{summary.evidence_snapshots.length === 0 ? 'No scenario states have been saved yet.' : `${summary.evidence_snapshots.length} scenario state${summary.evidence_snapshots.length === 1 ? '' : 's'} saved.`} {summary.execution.verdict === null ? <>The formal result remains <strong>not assessed</strong> until the approved comparison process is completed.</> : <>The approved comparison process is complete: <strong>{summary.execution.verdict}</strong>.</>}</p>
