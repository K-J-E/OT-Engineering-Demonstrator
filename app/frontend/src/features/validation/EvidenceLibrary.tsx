  ...
  return <div className="view-stack">
    <section className="panel" aria-labelledby="evidence-library-title">
      ...
      {projection.validation.library_executions.length === 0 ? <p className="empty-state">No validation execution records are available.</p> : <div className="record-list">{projection.validation.library_executions.map((summary) => {
        ...
        return <article key={summary.execution.validation_execution_id}>
          ...
          <dl><div><dt>Execution</dt><dd>{summary.execution.validation_execution_id}</dd></div><div><dt>Scenario run</dt><dd>{summary.execution.scenario_run_id}</dd></div><div><dt>Source catalogue</dt><dd>v{summary.execution.catalogue_version} · {summary.execution.catalogue_sha256}</dd></div><div><dt>Configuration</dt><dd>{summary.execution.configuration_id}</dd></div><div><dt>Source build</dt><dd>{summary.execution.application_build_id}</dd></div><div><dt>Started</dt><dd>{formatTime(summary.execution.started_scenario_time)}</dd></div><div><dt>Determination</dt><dd>{summary.execution.verdict ?? 'NOT DETERMINED'}</dd></div></dl>
    ...
    <section className="panel" aria-labelledby="package-register-title">
      ...
      {packages.length === 0 ? <p className="empty-state">No package has been generated. Every future export receives a new identity and path.</p> : <div className="record-list">{packages.map((item) => <article key={item.package_id}>
        ...
        <h3>{item.package_id}</h3><dl><div><dt>Source execution</dt><dd>{item.validation_execution_id}</dd></div><div><dt>Source configuration</dt><dd>{item.configuration_id} · v{item.configuration_version}</dd></div><div><dt>Source build</dt><dd>{item.application_build_id}</dd></div><div><dt>Generation build</dt><dd>{item.generation_application_build_id}</dd></div><div><dt>Archive path</dt><dd>{item.archive_path}</dd></div><div><dt>Manifest SHA-256</dt><dd>{item.manifest_sha256}</dd></div><div><dt>Archive SHA-256</dt><dd>{item.archive_sha256}</dd></div><div><dt>Source links</dt><dd>{item.source_record_references.join(', ')}</dd></div></dl>
