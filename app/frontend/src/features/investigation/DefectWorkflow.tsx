...
export function DefectResults({ projection, investigation, busy, onInvestigate }: {
  ...
  return <div className="view-stack defect-results">
    <section className="panel defect-results-intro" aria-labelledby="defect-results-title">
      ...
      <p>The operating mechanism has faithfully used the switch positions, telemetry and network connectivity supplied by GIS configuration v1.0. Its live controls can therefore pass—even treating one open boundary switch as sufficient isolation—although one seeded connection in that source configuration is wrong.</p>
      ...
      <div className="hidden-defect-explanation"><strong>Why both statements can be true</strong><p>Assurance asks whether the operating logic safely followed the information available during the run. In this case, assurance passes because the configured topology leads the operating logic to derive that no suitable alternate-supply candidate exists. Independent validation shows that this result is incorrect against the accepted outcome, indicating an error somewhere in the information chain and prompting the investigation.</p></div>
    </section>
...
export function CorrectedRunResult({ initial, projection, onReturnToOperational, onReturnToSelection, onRestartDefect }: {
  ...
  return <div className="view-stack corrected-result-view">
    <section className="panel" aria-labelledby="corrected-repeat-title">
      <div className="panel-heading"><div><span className="eyebrow">Corrected full-scenario repeat</span><h2 id="corrected-repeat-title">Prove the correction through the complete operating sequence</h2></div><div className="corrected-result-heading-actions">{complete && <button type="button" className="secondary-action" onClick={onReturnToOperational}>View the final network state</button>}<span className={`status-badge ${verdict === 'PASS' ? 'success' : 'neutral'}`}>{verdict}</span></div></div>
      ...
      {!complete && <div className="corrected-repeat-start"><div><strong>{correctedReady ? 'Review the corrected network before the full repeat' : 'Focused correction check required'}</strong><p>{correctedReady ? 'The corrected post-trip result now matches the accepted 850-customer outcome. Return to the operational view to inspect configuration v1.1 and start the complete corrected scenario.' : 'Complete the investigation, record the correction and pass the focused post-trip check first.'}</p></div><button type="button" className="primary-action defect-action" disabled={!correctedReady} onClick={onReturnToOperational}>Return to corrected network</button></div>}
      {complete && <>
        ...
        <details className="technical-details"><summary>Corrected repeat identity and technical traceability</summary><dl className="identity-grid"><div><dt>Evidence record</dt><dd>{regression.execution.validation_execution_id}</dd></div><div><dt>Defect</dt><dd>{workspace.defect_record?.defect_id}</dd></div><div><dt>Correction</dt><dd>{workspace.correction_record?.correction_id}</dd></div><div><dt>Configuration</dt><dd>{regression.execution.configuration_id} · v{regression.execution.configuration_version}</dd></div></dl></details>
        ...
        <div className="guided-continuation corrected-continuation"><div><strong>Corrected repeat complete</strong><p>The failed evidence, investigation and corrected PASS remain preserved. Return to the walkthrough selection when you are ready to review another case.</p></div><button type="button" className="primary-action" onClick={onReturnToSelection}>Return to walkthrough selection</button></div>
        <div className="guided-continuation defect-restart-control"><div><strong>Repeat the complete defect demonstration</strong><p>Restart from the seeded GIS error in configuration v1.0. This clears all locally generated runs, validation records and downloadable packages; controlled source configurations and application files are unchanged.</p></div>{!restartConfirmationVisible
          ? <button type="button" className="secondary-action" onClick={() => setRestartConfirmationVisible(true)}>Restart from the seeded defect</button>
          : <div className="restart-confirmation"><button type="button" className="secondary-action" disabled={restarting} onClick={() => setRestartConfirmationVisible(false)}>Keep current records</button><button type="button" className="primary-action defect-action" disabled={restarting} onClick={async () => { setRestarting(true); try { await onRestartDefect() } finally { setRestarting(false) } }}>{restarting ? 'Resetting local demonstration…' : 'Confirm restart and clear local records'}</button></div>}</div>
      </>}
    </section>
    <section className="panel hidden-defect-summary"><span className="eyebrow">What the defect case proves</span><h2>A plausible operating display is not enough</h2><p>Both configurations allowed the operating engine to calculate a coherent answer. Independent validation exposed the seeded v1.0 topology error; the controlled correction then brought the same engine back to the accepted result without changing its algorithms.</p></section>
  </div>
}
