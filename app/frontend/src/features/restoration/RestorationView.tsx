  ...
  return (
    <div className="view-stack">
      <section className={`panel outcome-card outcome-${assessment.outcome.toLowerCase()}`} aria-labelledby="restoration-title">
        ...
        <dl className="identity-grid"><div><dt>Run ID</dt><dd>{projection.run.scenario_run_id}</dd></div><div><dt>Network Configuration</dt><dd>{projection.run.configuration_id} · v{projection.run.configuration_version}</dd></div><div><dt>Assessment ID</dt><dd>{assessment.assessment_id}</dd></div><div><dt>Sequence</dt><dd>{assessment.assessment_sequence}</dd></div><div><dt>State revision binding</dt><dd>{assessment.state_revision}</dd></div><div><dt>Scenario time</dt><dd>{formatTime(assessment.scenario_time)}</dd></div><div><dt>Bound telemetry evidence</dt><dd>{assessment.telemetry_snapshot_sha256}</dd></div></dl>
