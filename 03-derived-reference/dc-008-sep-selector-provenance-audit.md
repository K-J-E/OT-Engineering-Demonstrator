...
|---|---|---|---|
| `SEP-01` | Run/definition both identify the same fixed formal section; every other configured section receives the actual boundary rejection; run unchanged | One configured alternate is accepted, definition differs from run, or evidence class is not FORMAL | Definition unresolved, candidate set incomplete, rejection missing/non-unique, or before/after run proof absent |
...
| `SEP-03` | Actual mutation attempts are rejected and run identity/hash remains unchanged; accepted switch creates separate run | Boundary accepts a mutation or existing run fields change | Either rejection outcome or before/after run evidence absent |
